...
class Trainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch + 1):
            ...
            print(
                f"\nrank: {self.local_rank}, time_escaped_epoch: {time_escaped:.3f} hours, estimated to finish"
                f" {self.max_epoch} epoch: {(self.max_epoch-epoch)*time_escaped:.3f}\n"
            )
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.dataloader_train):
            ...
            if (batch_idx + 1) % self.log_interval == 0 or (batch_idx + 1) == len(self.dataloader_train):
                ...
                gpu_info = "GPU, memory: {:.3f} GB, {:.3f} GB, {:.3f} GB, {:.3f} GB".format(
