    ...
    for key, value in src_state.items():
        if key in dst_state and (dst_state[key].size() == src_state[key].size()):
            ...
            if key not in dst_state:
                logging.warning(f"Filter out {key} from pretrained dict" + " because of name not found in target dict")
            else:
                logging.warning(
...
def assigment_scope_map(dst_state: dict, src_state: dict, scope_map: str = None):
    ...
    if scope_map is not None:
        ...
        for scope_map_idx in range(scope_map_num):
            ...
            logging.info(
                "assignment_map from scope {} to {}".format(scope_map[scope_map_id], scope_map[scope_map_id + 1])
            )
