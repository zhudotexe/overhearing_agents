...
class ParaformerStreaming(Paraformer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.ctc_weight > 0.0:
            ...
            loss_ctc, cer_ctc = self._calc_ctc_loss(encoder_out_ctc, encoder_out_lens_ctc, text, text_lengths)
    ...
    def _calc_att_predictor_loss(
        ...
        if self.encoder.overlap_chunk_cls is not None and self.decoder_attention_chunk_type == "chunk":
            ...
            mask_shift_att_chunk_decoder = self.encoder.overlap_chunk_cls.get_mask_shift_att_chunk_decoder(
                None, device=encoder_out.device, batch_size=encoder_out.size(0)
            )
        ...
        # 1. Forward decoder
        decoder_outs = self.decoder(encoder_out, encoder_out_lens, sematic_embeds, ys_pad_lens, scama_mask)
    ...
    def cal_decoder_with_predictor(self, encoder_out, encoder_out_lens, sematic_embeds, ys_pad_lens):
        decoder_outs = self.decoder(encoder_out, encoder_out_lens, sematic_embeds, ys_pad_lens, self.scama_mask)
        ...
        **kwargs,
        ...
        for i in range(b):
            ...
            for nbest_idx, hyp in enumerate(nbest_hyps):
                ...
                token_int = list(
                    filter(
                        lambda x: x != self.eos and x != self.sos and x != self.blank_id,
                        token_int,
                    )
                )
