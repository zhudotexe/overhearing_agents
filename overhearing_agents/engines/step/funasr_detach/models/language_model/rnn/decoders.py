...
from funasr_detach.models.transformer.utils.nets_utils import mask_by_length, pad_list, to_device
    ...
    def rnn_forward(self, ey, z_list, c_list, z_prev, c_prev):
        if self.dtype == "lstm":
            ...
            for i in six.moves.range(1, self.dlayers):
                z_list[i], c_list[i] = self.decoder[i](self.dropout_dec[i - 1](z_list[i - 1]), (z_prev[i], c_prev[i]))
        else:
    ...
    def recognize_beam_batch(
        ...
        for i in six.moves.range(maxlen):
            ...
            if ctc_scorer[0]:
                ...
                part_ids = torch.topk(local_scores, scoring_num, dim=-1)[1] if scoring_num > 0 else None
                for idx in range(self.num_encs):
                    att_w = att_w_list[idx]
                    att_w_ = att_w if isinstance(att_w, torch.Tensor) else att_w[0]
                    local_ctc_scores, ctc_state[idx] = ctc_scorer[idx](yseq, ctc_state[idx], part_ids, att_w_)
            ...
            accum_padded_beam_ids = (accum_best_ids // self.odim + pad_b).view(-1).data.cpu().tolist()
