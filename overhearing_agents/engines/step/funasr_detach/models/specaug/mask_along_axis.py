...
class MaskAlongAxis(torch.nn.Module):
    ...
    def extra_repr(self):
        return f"mask_width_range={self.mask_width_range}, num_mask={self.num_mask}, axis={self.mask_axis}"
...
class MaskAlongAxisVariableMaxWidth(torch.nn.Module):
    ...
    def __init__(
        ...
        if len(mask_width_ratio_range) != 2:
            raise TypeError(
                f"mask_width_ratio_range must be a tuple of float and float values: {mask_width_ratio_range}",
            )
    ...
    def extra_repr(self):
        return f"mask_width_ratio_range={self.mask_width_ratio_range}, num_mask={self.num_mask}, axis={self.mask_axis}"
