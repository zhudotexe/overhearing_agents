...
class WKVLinearAttentionEncoder(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            batch * dim % min(dim, 32) == 0
        ), f"batch size ({batch}) by dimension ({dim}) should be a multiple of {min(dim, 32)}"
...
def load_encoder_wkv_kernel(context_size: int) -> None:
    ...
    if not torch.cuda.is_available():
        raise ImportError(
            "CUDA is currently a requirement for WKV kernel loading. Please set your devices properly and launch again."
        )
...
class SelfAttention(torch.nn.Module):
    ...
    def reset_parameters(self, size: int, attention_size: int, block_id: int, num_blocks: int) -> None:
        ...
        decay_speed = [-5 + 8 * (h / (attention_size - 1)) ** (0.7 + 1.3 * ratio_0_to_1) for h in range(attention_size)]
        decay_speed = torch.tensor(decay_speed, dtype=self.time_decay.dtype, device=self.time_decay.device)
        ...
        with torch.no_grad():
            ...
            self.time_mix_value.data = torch.pow(time_weight, ratio_1_to_almost0) + 0.3 * ratio_0_to_1
