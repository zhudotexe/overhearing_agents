...
from funasr_detach.losses.label_smoothing_loss import (  # noqa: H301
    LabelSmoothingLoss,
    NllLoss,
)
...
class SAASRModel(FunASRModel):
    ...
    def __init__(
        ...
        if self.asr_encoder.interctc_use_conditioning:
            self.asr_encoder.conditioning_layer = torch.nn.Linear(vocab_size, self.asr_encoder.output_size())
        ...
        if report_cer or report_wer:
            self.error_calculator = ErrorCalculator(token_list, sym_space, sym_blank, report_cer, report_wer)
    ...
    def forward(
        ...
        if self.ctc_weight != 0.0:
            loss_ctc, cer_ctc = self._calc_ctc_loss(asr_encoder_out, encoder_out_lens, text, text_lengths)
        ...
        if self.interctc_weight != 0.0 and intermediate_outs is not None:
            for layer_idx, intermediate_out in intermediate_outs:
                ...
                loss_ic, cer_ic = self._calc_ctc_loss(intermediate_out, encoder_out_lens, text, text_lengths)
                loss_interctc = loss_interctc + loss_ic

                # Collect Intermedaite CTC stats
                stats["loss_interctc_layer{}".format(layer_idx)] = loss_ic.detach() if loss_ic is not None else None
    ...
    def _extract_feats(self, speech: torch.Tensor, speech_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def nll(
        ...
        # 1. Forward decoder
        decoder_out, _ = self.decoder(encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens)  # [batch, seqlen, dim]
