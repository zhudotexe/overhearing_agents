...
def postprocess(segments: list, vad_segments: list, labels: np.ndarray, embeddings: np.ndarray) -> list:
...
class File(object):
    ...
    def _get_storage(uri):
        ...
        assert (
            storage_type in File._prefix_to_storage
        ), f"Unsupported uri {uri}, valid prefixs: {list(File._prefix_to_storage.keys())}"
