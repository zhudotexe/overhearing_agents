...
from funasr_detach.models.transformer.utils.nets_utils import make_pad_mask, to_device
...
class DiarSondModel(FunASRModel):
    ...
    def rand_permute_speaker(self, raw_profile, raw_binary_labels):
        ...
        """
        assert raw_profile.shape[1] == raw_binary_labels.shape[2], "Num profile: {}, Num label: {}".format(
    ...
    def forward(
        ...
        if self.label_aggregator is not None:
            binary_labels, binary_labels_lengths = self.label_aggregator(binary_labels, binary_labels_lengths)
        # 0b. augment profiles
    ...
    def speaker_discrimination_loss(self, profile: torch.Tensor, profile_lengths: torch.Tensor) -> torch.Tensor:
        ...
        eps = 1e-12
        coding_norm = (
            torch.linalg.norm(profile * profile_mask + (1 - profile_mask) * eps, dim=2, keepdim=True) * profile_mask
        )
        # profile: Batch, N, dim
    ...
    def _extract_feats(self, speech: torch.Tensor, speech_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        batch_size = speech.shape[0]
        speech_lengths = (
            speech_lengths if speech_lengths is not None else torch.ones(batch_size).int() * speech.shape[1]
        )
