...
def statistic_pooling(xs_pad: torch.Tensor, ilens: torch.Tensor = None, pooling_dim: Tuple = (2, 3)) -> torch.Tensor:
    # xs_pad in (Batch, Channel, Time, Frequency)
    ...
    mean = torch.sum(xs_pad, dim=pooling_dim, keepdim=True) / torch.sum(seq_mask, dim=pooling_dim, keepdim=True)
    squared_difference = torch.pow(xs_pad - mean, 2.0)
