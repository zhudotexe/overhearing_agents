        ...
        if self.use_two_branches:
            if self.merge_method == "concat":
                ...
                if (
                    ...
                    if mask is not None:
                        min_value = float(numpy.finfo(torch.tensor(0, dtype=score1.dtype).numpy().dtype).min)
                    ...
                    if mask is not None:
                        min_value = float(numpy.finfo(torch.tensor(0, dtype=score2.dtype).numpy().dtype).min)
...
class BranchformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        if len(cgmlp_weight) != num_blocks:
            raise ValueError(
                f"Length of cgmlp_weight ({len(cgmlp_weight)}) should be equal to num_blocks ({num_blocks})"
            )
