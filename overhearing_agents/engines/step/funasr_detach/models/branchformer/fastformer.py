...
class FastSelfAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if size % attention_heads != 0:
            raise ValueError(f"Hidden size ({size}) is not an integer multiple of attention heads ({attention_heads})")
    ...
    def forward(self, xs_pad, mask):
        ...
        if mask is not None:
            min_value = float(numpy.finfo(torch.tensor(0, dtype=query_for_score.dtype).numpy().dtype).min)
        ...
        if mask is not None:
            min_value = float(numpy.finfo(torch.tensor(0, dtype=query_key_score.dtype).numpy().dtype).min)
