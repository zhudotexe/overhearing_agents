...
def stft(data, frame_size=1024, frame_shift=256):
    ...
    if len(data) % frame_shift == 0:
        return librosa.stft(data, n_fft=fft_size, win_length=frame_size, hop_length=frame_shift).T[:-1]
    else:
...
def get_frame_labels(kaldi_obj, rec, start=0, end=None, frame_size=1024, frame_shift=256, n_speakers=None):
