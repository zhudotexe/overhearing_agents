...
from funasr_detach.models.eend.utils.power import create_powerlabel, generate_mapping_dict
        ...
        if y.shape[1] < out_size:
            ys_padded.append(
                torch.cat(
                    [
                        y,
                        torch.zeros(y.shape[0], out_size - y.shape[1]).to(torch.float32).to(y.device),
                    ],
                    dim=1,
                )
            )
...
class DiarEENDOLAModel(nn.Module):
    ...
    def estimate_sequential(
        ...
        **kwargs,
        ...
        ys = [
            self.recover_y_from_powerlabel(logit, raw_n_speaker) for logit, raw_n_speaker in zip(logits, raw_n_speakers)
        ]
    ...
    def recover_y_from_powerlabel(self, logit, n_speaker):
        ...
        decisions = (
            torch.from_numpy(np.stack([np.array([int(i) for i in dec]) for dec in decisions], axis=0))
            .to(logit.device)
            .to(torch.float32)
        )
