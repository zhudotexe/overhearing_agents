...
from funasr_detach.models.emotion2vec.fairseq_modules import GradMultiply, compute_mask_indices, index_put
...
class ModalitySpecificEncoder(nn.Module):
            ...
            self.alibi_scale = nn.Parameter(
                torch.full(
                    (
                        ...
                        1,
                        (self.modality_cfg.num_alibi_heads if modality_cfg.learned_alibi_scale_per_head else 1),
                        1,
                        1,
                    ),
        ...
        if mask:
            if clone_batch > 1:
                ...
                if mask_seeds is not None:
                    clone_hash = [int(hash((mask_seeds.seed, ind)) % 1e10) for ind in range(clone_batch - 1)]
    ...
    def apply_mask(self, x, mask_info):
        ...
        if cfg.mask_channel_prob > 0:
            ...
            mask_channel = torch.from_numpy(mask_channel).to(x.device).unsqueeze(1).expand(-1, T, -1)
...
def get_alibi_bias(
    ...
    if (
        ...
        buffered = get_alibi(bt, heads, dims=dims, distance=distance).to(dtype=dtype, device=device).repeat(bn, 1, 1)
