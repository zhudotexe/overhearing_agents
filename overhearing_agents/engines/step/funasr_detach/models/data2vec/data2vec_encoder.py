...
class Data2VecEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.num_updates > 5000 and result["target_var"] < self.min_target_var:
            logging.error(f"target var is {result['target_var'].item()} < {self.min_target_var}, exiting")
            raise Exception(f"target var is {result['target_var'].item()} < {self.min_target_var}, exiting")
        if self.num_updates > 5000 and result["pred_var"] < self.min_pred_var:
            ...
            raise Exception(f"pred var is {result['pred_var'].item()} < {self.min_pred_var}, exiting")
