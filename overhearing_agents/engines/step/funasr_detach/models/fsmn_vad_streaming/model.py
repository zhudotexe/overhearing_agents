...
class FsmnVADStreaming(nn.Module):
    ...
    def ComputeDecibel(self, cache: dict = {}) -> None:
        ...
        for offset in range(
            ...
            cache["stats"].decibel.append(
                10
                * math.log10(
                    (cache["stats"].waveform[0][offset : offset + frame_sample_length]).square().sum() + 0.000001
                )
            )
        ...
        if math.exp(speech_prob) >= math.exp(noise_prob) + cache["stats"].speech_noise_thres:
            ...
            if cache["stats"].noise_average_decibel < -99.9:
                ...
                cache["stats"].noise_average_decibel = (
                    cur_decibel
                    + cache["stats"].noise_average_decibel * (self.vad_opts.noise_frame_num_used_for_snr - 1)
    ...
    def inference(
        ...
        **kwargs,
        ...
        is_streaming_input = (
            kwargs.get("is_streaming_input", False) if chunk_size >= 15000 else kwargs.get("is_streaming_input", True)
        )
