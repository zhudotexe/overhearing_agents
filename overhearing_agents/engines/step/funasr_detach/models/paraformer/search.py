...
class BeamSearchPara(torch.nn.Module):
    ...
    def search(self, running_hyps: List[Hypothesis], x: torch.Tensor, am_score: torch.Tensor) -> List[Hypothesis]:
    ...
    def forward(
        ...
        if self.token_list is not None:
            logging.info("best hypo: " + "".join([self.token_list[x.item()] for x in best.yseq[1:-1]]) + "\n")
        return nbest_hyps
    ...
    def post_process(
        ...
        if self.token_list is not None:
            logging.debug("best hypo: " + "".join([self.token_list[x.item()] for x in running_hyps[0].yseq[1:]]))
