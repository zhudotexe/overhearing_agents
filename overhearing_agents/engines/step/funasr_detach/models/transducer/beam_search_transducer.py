...
class BeamSearchTransducer:
    ...
    def __init__(
        ...
        if search_type == "default":
            ...
            assert (
                self.vocab_size >= beam_size + expansion_beta
            ), "beam_size (%d) + expansion_beta (%d)  should be smaller than or equal to vocab size (%d)." % (
                beam_size,
                expansion_beta,
                self.vocab_size,
            )
    ...
    def modified_adaptive_expansion_search(
        ...
        for enc_out_t in enc_out:
            ...
            for n in range(self.nstep):
                ...
                if not list_exp:
                    kept_hyps = sorted(self.recombine_hyps(list_b), key=lambda x: x.score, reverse=True)[
                        : self.beam_size
                    ]
