...
class S3prlFrontend(nn.Module):
    ...
    def _get_upstream(self, frontend_conf):
        ...
        if getattr(s3prl_upstream, "model", None) is not None and s3prl_upstream.model.__class__.__name__ in [
            "Wav2Vec2Model",
            "HubertModel",
        ]:
    ...
    def _tile_representations(self, feature):
        ...
        """
        assert len(feature.shape) == 3, "Input argument `feature` has invalid shape: {}".format(feature.shape)
        tiled_feature = feature.repeat(1, 1, self.args.tile_factor)
        tiled_feature = tiled_feature.reshape(feature.size(0), feature.size(1) * self.args.tile_factor, feature.size(2))
