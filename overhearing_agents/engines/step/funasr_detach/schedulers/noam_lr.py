...
class NoamLR(_LRScheduler, AbsBatchStepScheduler):
    ...
    def __init__(
        ...
        warnings.warn(
            f"NoamLR is deprecated. Use WarmupLR(warmup_steps={warmup_steps}) with Optimizer(lr={new_lr})",
        )
    ...
    def get_lr(self):
        ...
        return [
            lr * self.model_size**-0.5 * min(step_num**-0.5, step_num * self.warmup_steps**-1.5) for lr in self.base_lrs
        ]
