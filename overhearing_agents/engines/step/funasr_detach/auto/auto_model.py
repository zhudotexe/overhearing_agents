...
from funasr_detach.models.campplus.utils import distribute_spk, postprocess, sv_chunk
...
from funasr_detach.utils.load_utils import load_audio_text_image_video, load_bytes
...
class AutoModel:
    ...
    def inference_with_vad(self, input, input_len=None, **cfg):
        ...
        for i in range(len(res)):
            ...
            for j, _ in enumerate(range(0, n)):
                ...
                if (
                    j < n - 1
                    and (batch_size_ms_cum + sorted_data[j + 1][0][1] - sorted_data[j + 1][0][0]) < batch_size
                    and (sorted_data[j + 1][0][1] - sorted_data[j + 1][0][0]) < batch_size_threshold_ms
                ):
                    continue
                batch_size_ms_cum = 0
                end_idx = j + 1
                ...
                if self.spk_mode == "vad_segment":  # recover sentence_list
                    ...
                    for res, vadsegment in zip(restored_data, vadsegments):
                        if "timestamp" not in res:
                            logging.error(
                                ...
                                "                                    can predict timestamp, and speaker diarization"
                    ...
                    if "timestamp" not in result:
                        logging.error(
                            ...
                            "                                    and"
                            " 'iic/speech_seaco_paraformer_large_asr_nat-zh-cn-16k-common-vocab8404-pytorch'           "
                            "                            can predict timestamp, and speaker diarization relies on"
                            " timestamps."
                        )
    ...
    def infer_encoder(self, input, input_len=None, model=None, kwargs=None, key=None, **cfg):
        kwargs = self.kwargs if kwargs is None else kwargs
        kwargs.update(cfg)
        model = self.model if model is None else model
        model = model.cuda()
